...
## Data & Analytics > Log & Crash Search > Logback SDK 사용 가이드 { #data-analytics-log-crash-search-logback-sdk-guide }
...
## 1. Log & Crash Logback SDK 추가 { #add-log-crash-logback-sdk }
...
## 2. Log & Crash Logback SDK에 필요한 의존성 추가 { #add-dependency-for-log-crash-logback-sdk }

<a id="1-install-maven"></a>
### 2.1 Maven 설치 { #1-install-maven }
...
### 2.2 Gradle 설치 { #2-install-gradle }
...
## 3. Logger 설정 및 옵션 { #logger-setting-and-options }
...
### 3.1 Logback의 AsyncAppender 옵션 { #1-asyncappender-option-of-logback }
...
### 3.2 Log & Crash Logback SDK의 LogNCrashHttpAppender 옵션 { #2-logncrashhttpappender-option-of-log-crash-logback-sdk }
...
### 3.3 사용자 정의 옵션 { #3-user-defined-option }
...
## 4. LogNCrash SDK 사용 예 { #example-of-logncrash-sdk }
...
### Q. Log & Crash Logback SDK의 LogNCrashHttpAppender만으로 로그를 전송할 수는 없나요? { #q-can-i-send-logs-only-with-logncrashhttpappender-of-log-crash-logback-sdk }
...
### Q. batch program(project)에서 logncrash client를 사용하려면? { #q-how-can-i-use-logncrash-client-in-a-batch-program-project }
...
### Q. Java stack trace를 logback(Log & Crash Search 포함)에 로깅하려면? { #q-how-can-a-java-stack-trace-be-logged-to-a-logback-including-log-crash-search }
...
### Q. WAS 에서 사용시 안정적인 종료를 하려면? { #q-how-can-i-safely-close-was }
