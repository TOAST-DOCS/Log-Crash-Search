...
1. 쿼리 작성 후, 오른쪽의 x 버튼을 클릭하면 쿼리를 일괄 삭제할 수 있는 기능이 추가되었습니다.
2. 실시간 검색은 더 이상 제공하지 않습니다.
3. 다운로드 버튼이 따로 분리되었습니다. 크래시 덤프는 logType:"CRASH"로 검색하지 않아도 다운로드할 수 있습니다.
4. Chart의 Export 기능을 사용해 데이터를 저장할 수 있습니다.
5. 집계한 개수(count)는 선택한 필드를 클릭하면 별도 창에서 확인할 수 있습니다.
6. 테이블 형태로 확인할 수 있습니다. 선택한 필드에 따라 기본 표시 데이터가 달라지며, 다른 필드까지 함께 보려면, 오른쪽 펼치기 버튼을 클릭합니다.

* 그 외 변경 사항
    * 검색 소요 시간은 제공하지 않습니다.
    * 세션 로그는 기본 꺼짐(off) 고정으로 제공됩니다.
    * 저장한 로그 페이지가 별도 탭으로 분리되었습니다.
...
|의견 및 히스토리| 해당 이슈에 대해 코멘트를 작성할 수 있으며 프로젝트 구성원들이 작성한 코멘트 목록을 보여 주며, 해당 이슈에 대한 관리 이력(상태 변경, 태그 추가, 코멘트 작성 등)을 타임라인으로 보여 줍니다|
...
|크래시 지표| 검색 기간 내 앱 버전별 세션/크래시 수/발생률(%)/이전 기간 크래시/증가율(%) 정보를 제공합니다.|
...
|---|---|
|시간 조건| 통계 시간 조건을 설정할 수 있습니다. |
|검색 조건| 오류 유형 - 시스템에서 발생 시키는 Crash 통계와 Exception 블럭에서 사용자가 발생시키는 Handled로 구분 <br/> 플랫폼 - 앱 플랫폼 <br/> 앱 버전 - 앱 버전 <br/> 사용자 - SDK에서 user_id로 지정한 사용자 아이디 <br/> 기기 - 디바이스명|
...
|---|---|
|시간 조건|	통계 시간 조건을 설정할 수 있습니다.|
|검색 조건|	오류 유형 - 시스템에서 발생 시키는 Crash 통계와 Exception 블럭에서 사용자가 발생시키는 Handled로 구분 <br/> 플랫폼 - 앱 플랫폼 <br/> 앱 버전 - 앱 버전 <br/> 국가 - 이슈가 발생한 사용자의 국가 <br/> 운영체제 버전 - 이슈가 발생된 디바이스의 운영체제 버전|
